...
CONTAINER=$(docker ps --filter name=serial-web --format '{{.Names}}' | head -1)
BACKUP_DIR="$HOME/backups/serial/$(date +%Y-%m-%d_%H%M)"
NAS_DEST="admin@nas:/share/backups/serial"
B2_DEST="b2:serial-backups"

if [ -z "$CONTAINER" ]; then
  echo "ERROR: No serial container running"
  exit 1
fi
...
rsync -az --delete "$HOME/backups/serial/" "$NAS_DEST/" 2>/dev/null || echo "WARNING: NAS sync failed"

# Sync to Backblaze B2 (offsite, disaster recovery)
rclone sync "$HOME/backups/serial/" "$B2_DEST/" 2>/dev/null || echo "WARNING: B2 sync failed"

# Prune local backups older than 7 days
find "$HOME/backups/serial" -maxdepth 1 -type d -mtime +7 -exec rm -rf {} +

# Prune B2 backups older than 30 days
rclone delete --min-age 30d "$B2_DEST/" 2>/dev/null || echo "WARNING: B2 prune failed"

echo "$(date): Serial backup complete → $BACKUP_DIR"
